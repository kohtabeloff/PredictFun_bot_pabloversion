...
10. [Как обновить бота до последней версии](#10-как-обновить-бота-до-последней-версии)
11. [Основные параметры](#11-основные-параметры)
...
Прокси указывается в настройках бота (**🔑 Аккаунт**) — поле **Прокси**. Формат:
...
1. Нажми **🔑 Аккаунт** в правом верхнем углу
2. Заполни поля:
...
Парсер встроен прямо в **Менеджер** — отдельный инструмент устанавливать не нужно. Открой Менеджер (`http://YOUR_SERVER_IP:8000`) и перейди на вкладку **Парсер**.

### Первоначальная настройка

При первом использовании введи свой API ключ Predict.fun в поле **API Key** и нажми **Сохранить** — ключ сохранится и больше вводить не потребуется.

### Настройки парсера

**Источник маркетов**
- *Все маркеты Predict.fun* — загружает полный список с платформы (рекомендуется)
- *Ввести ID вручную* — введи список ID через запятую или с новой строки, если хочешь проверить конкретные маркеты

**Исключить маркеты с тегами**
Отмечай категории, которые хочешь **убрать** из результата. Например, не хочешь фармить спортивные маркеты — поставь галочку *Sports*. Список тегов загружается автоматически с платформы и всегда актуален.

**Дополнительные фильтры**
- *Статус маркета* — по умолчанию `REGISTERED` (активные маркеты). Обычно менять не нужно.
- *Фильтр Polymarket / Kalshi* — оставляет только маркеты, которые одновременно есть на других крупных платформах. Такие маркеты обычно популярнее и ликвиднее.
- *Мин. дней до конца* — отсекает маркеты, которые закрываются слишком скоро (работает совместно с фильтром Polymarket/Kalshi).

### Запуск и результат

Нажми **Запустить парсер** — прогресс отображается в реальном времени по шагам. После завершения появится список найденных маркетов.

**Что делать с результатом:**
- **Добавить в бот** — выбери нужного бота из списка и нажми кнопку: маркеты добавятся напрямую, старые маркеты у бота при этом останутся
- **Скачать .txt** — сохранит список ID в файл, который можно добавить в бота через **Добавить маркеты** вручную
...
| **⬇ Экспорт списка (.txt)** | Скачивает все ID маркетов в текстовый файл |
| **⬇ Экспорт с настройками (.json)** | Скачивает маркеты вместе со всеми настройками каждого — удобно для переноса между аккаунтами |
| **⬆ Импорт (.json)** | Загружает маркеты из ранее сохранённого `.json` файла |
| **⚡ Менеджер** | Открывает общий дашборд всех ботов |
| **🔑 Аккаунт** | Настройки аккаунта: API ключ, адрес, приватный ключ, Telegram, прокси, пароль UI |
| **⚙ Параметры** | Параметры бота: фильтр поинтов, авто-продажа позиций |

### Рабочий процесс

1. Нажми **СТАРТ** — бот подключается к платформе и восстанавливает сохранённые маркеты
2. Используй парсер (раздел 8) чтобы получить актуальные ID маркетов
...
6. Нажми **Запустить все** — бот начнёт выставлять ордера на всех маркетах
7. Для экспорта текущего списка — кнопка **⬇ Экспорт списка** в шапке
...
4. Введи chat_id в настройках бота (**🔑 Аккаунт**) — поле **Chat ID**
...
## 10. Как обновить бота до последней версии

Подключись к серверу по SSH. Сначала найди папку с ботом — это папка, в которую ты клонировал репозиторий при установке:

```bash
ls ~
```

Перейди в неё (замени `bot` на своё название папки) и выполни:
...
Торговый бот при этом **не останавливается** — он продолжает работать. Перезапускается только Менеджер (интерфейс на порту 8000), чтобы подхватить новые файлы.

> Если в обновлении изменился основной код бота (`main.py`, `core/`, `api/`) — перезапусти и его: `sudo systemctl restart predictfun-bot.service`. Это кратковременно остановит торговлю и отменит активные ордера.
